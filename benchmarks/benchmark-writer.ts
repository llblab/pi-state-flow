...
process.on("message", (message: { scope: "session" | "global"; round: number } | "stop") => {
	...
	try {
		...
		runtime.publish(candidate, true, accepted);
		snapshot = candidate;
		process.send!({ accepted: true, ms: performance.now() - started, step: snapshot.meta.step });
	} catch (error) {
		process.send!({ accepted: false, ms: performance.now() - started, error: error instanceof Error ? error.message : String(error), step: snapshot.meta.step });
	}
});
