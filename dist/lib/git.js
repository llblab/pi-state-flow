    ...
    const visit = (directory, depth) => {
        ...
        for (const entry of readdirSync(directory, { withFileTypes: true })) {
            ...
            if (isStateFlowOwnedPath(path, root)) {
                if (!entry.isFile())
                    throw new Error(`State Flow backup source is not a regular file: ${path}`);
                paths.add(path);
            }
            else if (depth < 2 && isStateFlowOwnedPath(join(path, "checkpoint.json"), root)) {
                if (entry.isSymbolicLink())
                    throw new Error(`State Flow backup namespace is not a regular directory: ${path}`);
...
function prepareBackupWorktree(root, worktree, files) {
    mkdirSync(worktree);
    ...
    for (const directory of directories) {
        ...
        if (!stat.isFile() || stat.isSymbolicLink())
            throw new Error(`State Flow backup attributes are not a regular file: ${source}`);
...
function currentBranchRef(repositoryRoot) {
...
async function commitCurrentOwnedFiles(repositoryRoot, expectedHead, signal, waitForLock = true) {
    ...
    try {
        if (expectedHead === undefined)
            ...
            git(repositoryRoot, ["read-tree", expectedHead], { env });
        const owned = (paths) => paths.split("\0").filter((path) => path.length > 0 && isStateFlowOwnedPath(resolve(repositoryRoot, path), repositoryRoot));
        ...
        if (ignore.status !== 0 && (ignore.status !== 1 || ignore.stderr.trim()))
            throw new Error(`Git ignore selection failed: ${ignore.stderr || ignore.status}`);
        ...
        if (expectedHead !== undefined && tree === git(repositoryRoot, ["rev-parse", `${expectedHead}^{tree}`]).stdout.trim())
            return undefined;
        const args = ["commit-tree", tree];
        if (expectedHead !== undefined)
            args.push("-p", expectedHead);
