...
test("keeps validation attempt accounting across tool-bearing retry turns", async () => {
	...
	assert.equal(h.resolveSnapshot().meta.validation!.attempt, 2);
});
test("regenerates when a later handler removes the finalized response", async () => {
	...
	assert.equal(loadSessionState(h.ctx.cwd, "harness-session", h.repositoryRoot)!.response, "Recovered response");
});
test("regenerates invalid terminal responses without automatically disabling after the retry limit", async () => {
	...
	const committed = structuredClone(loadSessionState(h.ctx.cwd, "harness-session", h.repositoryRoot));
	...
	assert.equal(h.resolveSnapshot().meta.validation, undefined);
	assert.deepEqual(loadSessionState(h.ctx.cwd, "harness-session", h.repositoryRoot), committed);
});
