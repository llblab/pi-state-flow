...
import { adoptFileStateToGit, initializeGitRepository, isLocalGitRepository, captureTemporalGitBase, loadLegacyStatesAtRevision, loadTemporalRevision, migrateLegacyStorageToGit, publishTemporalStateToGit, type TemporalGitBase } from "./git.ts";
...
export function inspectRuntimeRevision(cwd: string, sessionId: string, root: string, revision: string) {
	const loaded = loadTemporalRevision(cwd, sessionId, root, revision);
	...
	return { runtime: loaded.runtime, resolved, view };
}

/** Both checkpoint generations validate their immutable target before any live migration/publication. */
export function inspectSnapshotRevision(cwd: string, sessionId: string, root: string, revision: string, legacySnapshot?: Snapshot) {
	if (isFileRevision(revision)) {
		const file = loadTemporalFileRevision(cwd, sessionId, root, revision);
	...
	try {
		const temporal = inspectRuntimeRevision(cwd, sessionId, root, revision);
		...
		const historical = loadLegacyStatesAtRevision(cwd, sessionId, root, revision);
...
export class TemporalRuntime {
	...
	constructor(cwd: string, sessionId: string, root: string) {
		this.cwd = cwd;
	...
	promote(snapshot: Snapshot): RuntimePublication | undefined {
		...
		const result = adoptFileStateToGit(this.cwd, this.sessionId, this.root, snapshot.meta.durableBase, snapshot);
	...
	restore(revision: string, legacySnapshot?: Snapshot): Snapshot {
		const inspected = inspectSnapshotRevision(this.cwd, this.sessionId, this.root, revision, legacySnapshot);
		if (inspected.file) {
		...
		const base = captureTemporalGitBase(this.cwd, this.sessionId, this.root);
		const reference = loaded.runtime.document.meta.temporalRevision;
		const semanticRevision = reference === undefined || reference === "self" ? loaded.runtime.revision : reference;
		const savedRuntime = hashJson(createSessionRuntime(resolved.snapshot, this.cwd, this.sessionId, view.lineage));
		resolved.snapshot.meta.pendingPublication = { commit: resolved.publicationTarget, error: "Durable publication intent is unconfirmed" };
		try {
	...
	initialize(snapshot: Snapshot, allowCreateCwd: boolean, expectedShared?: Pick<ScopedStates, "global" | "cwd">, newSessionOrigin = false): RuntimePublication | undefined {
		...
		if (backend === "git") migrateLegacyStorageToGit(this.cwd, this.sessionId, this.root);
		else {
			if (allowCreateCwd) initializeFileStore(this.root);
			migrateLegacyStorageToFiles(this.cwd, this.sessionId, this.root);
		}
		const base: TemporalGitBase = backend === "git" ? captureTemporalGitBase(this.cwd, this.sessionId, this.root) : captureTemporalFileBase(this.cwd, this.sessionId, this.root);
		const files = new Map(base.files.map((file) => [file.path, file.content]));
		const streams = Object.fromEntries(SCOPES.map((scope) => {
			const paths = temporalScopePaths(this.cwd, this.sessionId, scope, this.root);
		...
		const paths = sessionRuntimePaths(this.cwd, this.sessionId, this.root);
		...
		const candidate = new TemporalRuntime(this.cwd, this.sessionId, this.root);
	...
	publish(snapshot: Snapshot, semantic = false, accepted?: AcceptedTransition): RuntimePublication | undefined {
		...
		if (semantic && this.semanticRevision) {
			...
			for (const scope of ["global", "cwd"] as const) {
				const paths = temporalScopePaths(this.cwd, this.sessionId, scope, this.root);
				const stream = parseScopeStream(live.get(paths.checkpoint), live.get(paths.patches), scope);
		...
		if (this.backend === "files") {
			...
			const result = publishTemporalStateToFiles(this.cwd, this.sessionId, next, semantic ? SCOPES : [], this.base, this.root, runtime);
			this.base = result.base;
			this.view = next;
		...
		if (!semantic) {
			const current = captureTemporalGitBase(this.cwd, this.sessionId, this.root);
			const paths = sessionRuntimePaths(this.cwd, this.sessionId, this.root);
		...
		const result = publishTemporalStateToGit(this.cwd, this.sessionId, next, semantic ? SCOPES : [], this.base, this.root, runtime);
		this.base = result.base;
		this.view = next;
		this.savedRuntime = fingerprint;
